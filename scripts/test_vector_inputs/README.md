...
Each input file is a complete, production-shape `openproof.attestation_manifest.v1` document. It contains no documentation or annotation fields. The file is canonicalized byte-for-byte by the test vector generator. Any change to an input file changes the corresponding manifest hash in the test vector.

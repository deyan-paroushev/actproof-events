#!/usr/bin/env bash
# ============================================================================
# build-2.2.0.sh  —  ActProof Events 2.2.0 trust release, minted in Codespaces.
#
# 2.2.0 is a TRUST release: no new surface area. It adds, computed from the
...
#   1) place actproof-events-2.2.0-update.tar.gz next to this script
#   2) chmod +x build-2.2.0.sh
#   3) ./build-2.2.0.sh            # build + verify + tag, NO upload
#      ./build-2.2.0.sh --upload   # also twine upload to PyPI
# ============================================================================
set -euo pipefail

UPDATE_TARBALL="${UPDATE_TARBALL:-actproof-events-2.2.0-update.tar.gz}"
DO_UPLOAD="no"
[ "${1:-}" = "--upload" ] && DO_UPLOAD="yes"

EXPECT_VERSION="2.2.0"
BASE_VERSION="2.1.0"
...
# --- 1. apply the delta ------------------------------------------------------
say "1. apply 2.2.0 delta tarball"
...
say "3. verify 2.2.0 features"
python - <<'PY'
import actproof_events as ae, copy
from actproof_events.overlay_impact import build_overlay_impact_report
...
assert ae.__version__ == "2.2.0", ae.__version__
...
old = build_profile_view(ACT)
# unchanged via convenience path -> no_profile_change
r1 = build_overlay_impact_report(ov, old)
assert r1["impact_status"] == "no_profile_change", r1["impact_status"]
# change a targeted field -> impact flagged, reviewer preserved
tgt = ov["mappings"][0]["selected_actproof_field_id"]
new = copy.deepcopy(old)
for f in new["fields"]:
    if f["field_id"] == tgt:
        f["source_atoms"] = list(f.get("source_atoms", [])) + ["src.changed"]
new["profile_semantic_hash"] = "sha256:" + "c"*64
r2 = build_overlay_impact_report(ov, old, new)
assert r2["summary"]["accepted_mappings_impacted"] >= 1
assert r2["impacted_mappings"][0]["reviewed_by"] == "ICT Risk"
print(f"  version {ae.__version__} OK")
print(f"  overlay-impact: convenience=no-change, changed-field flagged, reviewer/date preserved OK")
PY
...
"$TMPV/bin/actproof-events" init-overlay-from-schema op:eu.dora.ict_incident_notification_initial.v1 examples/external-schema.example.json --institution "Smoke Bank" --out /tmp/sov.json && "$TMPV/bin/actproof-events" export-profile-view op:eu.dora.ict_incident_notification_initial.v1 --out /tmp/sold.json && "$TMPV/bin/actproof-events" diff-overlay-impact /tmp/sov.json /tmp/sold.json --json | head -8
rm -rf "$TMPV"
...
say "DONE — 2.2.0 built and verified."

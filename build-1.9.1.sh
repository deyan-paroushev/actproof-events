#!/usr/bin/env bash
# ============================================================================
# build-1.9.1.sh  —  ActProof Events 1.9.1 trust release, minted in Codespaces.
#
# 1.9.1 is a TRUST release: no new surface area. It adds, computed from the
...
#   1) place actproof-events-1.9.1-update.tar.gz next to this script
#   2) chmod +x build-1.9.1.sh
#   3) ./build-1.9.1.sh            # build + verify + tag, NO upload
#      ./build-1.9.1.sh --upload   # also twine upload to PyPI
# ============================================================================
set -euo pipefail

UPDATE_TARBALL="${UPDATE_TARBALL:-actproof-events-1.9.1-update.tar.gz}"
DO_UPLOAD="no"
...
# --- 1. apply the delta ------------------------------------------------------
say "1. apply 1.9.1 delta tarball"
...
say "3. verify 1.9.1 features"
python - <<'PY'
import actproof_events as ae
from actproof_events.profile_diff import diff_profile_views
from actproof_events.exports import build_profile_view
ACT = "op:eu.dora.ict_incident_notification_initial.v1"
assert ae.__version__ == "1.9.1", ae.__version__
v = build_profile_view(ACT, generated_at="x")
assert diff_profile_views(v, v)["summary"]["review_required"] is False
import copy
b = copy.deepcopy(v); b["completeness"]["review_status"] = "maintainer_reviewed"
d = diff_profile_views(v, b)
assert d["summary"]["review_required"] is True
print(f"  version {ae.__version__} OK")
print(f"  diff-profile: identical=no-review, review-status change detected OK")
PY
...
"$TMPV/bin/actproof-events" export-profile-view op:eu.dora.ict_incident_notification_initial.v1 --out /tmp/sv.json && cp /tmp/sv.json /tmp/sv2.json && "$TMPV/bin/actproof-events" diff-profile /tmp/sv.json /tmp/sv2.json | head -5
rm -rf "$TMPV"
...
say "DONE — 1.9.1 built and verified."

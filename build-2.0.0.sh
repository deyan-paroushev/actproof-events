#!/usr/bin/env bash
# ============================================================================
# build-2.0.0.sh  —  ActProof Events 2.0.0 trust release, minted in Codespaces.
#
# 2.0.0 is a TRUST release: no new surface area. It adds, computed from the
...
#   1) place actproof-events-2.0.0-update.tar.gz next to this script
#   2) chmod +x build-2.0.0.sh
#   3) ./build-2.0.0.sh            # build + verify + tag, NO upload
#      ./build-2.0.0.sh --upload   # also twine upload to PyPI
# ============================================================================
set -euo pipefail

UPDATE_TARBALL="${UPDATE_TARBALL:-actproof-events-2.0.0-update.tar.gz}"
DO_UPLOAD="no"
...
# --- 1. apply the delta ------------------------------------------------------
say "1. apply 2.0.0 delta tarball"
...
say "3. verify 2.0.0 features"
python - <<'PY'
import actproof_events as ae, os, tempfile
from actproof_events.profile_governance import build_governance_status, build_bank_poc_pack, list_review_records, _hash_record
ACT = "op:eu.dora.ict_incident_notification_initial.v1"
assert ae.__version__ == "2.0.0", ae.__version__
st = build_governance_status(ACT)
# honest scoping: profile stays candidate, 20 reviewed, 7 high-interpretation held
assert st["lifecycle_state"] == "candidate", f"profile should stay candidate, got {st['lifecycle_state']}"
assert len(st["reviewed_field_ids"]) == 20, st["reviewed_field_ids"]
assert len(st["held_at_draft_field_ids"]) == 7, st["held_at_draft_field_ids"]
# review record hash verifies
import copy
r = list_review_records(ACT)[0]
rr = copy.deepcopy(r); rr.pop("review_record_hash", None)
assert _hash_record(rr) == r["review_record_hash"], "review record hash failed"
# bank POC pack builds
d = tempfile.mkdtemp()
m = build_bank_poc_pack(ACT, out_dir=d)
assert m["bank_poc_pack_hash"].startswith("sha256:")
assert len(os.listdir(d)) >= 10
print(f"  version {ae.__version__} OK")
print(f"  governance: lifecycle=candidate, reviewed=20/27 (7 interpretive held), record verifies")
print(f"  bank POC pack: {len(os.listdir(d))} files, hash-bound OK")
PY
...
"$TMPV/bin/python" -c "from actproof_events.profile_governance import build_governance_status as g; s=g(\"op:eu.dora.ict_incident_notification_initial.v1\"); print(\"lifecycle:\", s[\"lifecycle_state\"], \"| reviewed:\", len(s[\"reviewed_field_ids\"]), \"| held:\", len(s[\"held_at_draft_field_ids\"]))"
rm -rf "$TMPV"
...
say "DONE — 2.0.0 built and verified."

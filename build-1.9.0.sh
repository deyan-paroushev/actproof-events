#!/usr/bin/env bash
# ============================================================================
# build-1.9.0.sh  —  ActProof Events 1.9.0 trust release, minted in Codespaces.
#
# 1.9.0 is a TRUST release: no new surface area. It adds, computed from the
...
#   1) place actproof-events-1.9.0-update.tar.gz next to this script
#   2) chmod +x build-1.9.0.sh
#   3) ./build-1.9.0.sh            # build + verify + tag, NO upload
#      ./build-1.9.0.sh --upload   # also twine upload to PyPI
# ============================================================================
set -euo pipefail

UPDATE_TARBALL="${UPDATE_TARBALL:-actproof-events-1.9.0-update.tar.gz}"
DO_UPLOAD="no"
[ "${1:-}" = "--upload" ] && DO_UPLOAD="yes"

EXPECT_VERSION="1.9.0"
BASE_VERSION="1.8.2"
...
# --- 1. apply the delta ------------------------------------------------------
say "1. apply 1.9.0 delta tarball"
...
say "3. verify 1.9.0 features"
python - <<'PY'
import actproof_events as ae
from actproof_events.schema_mapping import compare_schema
ACT = "op:eu.dora.ict_incident_notification_initial.v1"
assert ae.__version__ == "1.9.0", ae.__version__
r = compare_schema(ACT, ["affectedMemberStates", "internalEscalationOwner", "entityLEI"])
assert all(m["mapping_status"] == "candidate_review_required" for m in r["mappings"]), "non-candidate status!"
for m in r["mappings"]:
    for c in m.get("candidates", []):
        assert c["candidate_strength"] in {"weak", "medium", "strong"}
assert "high confidence" not in str(r).lower()
print(f"  version {ae.__version__} OK")
print(f"  compare-schema: external={r['summary']['external_field_count']} "
      f"unmapped={r['summary']['unmapped_external_field_count']} "
      f"ambiguous={r['summary']['ambiguous_mapping_count']}")
print(f"  all entries candidate_review_required: OK")
PY
...
echo '["affectedMemberStates"]' > /tmp/sf.json && "$TMPV/bin/actproof-events" compare-schema op:eu.dora.ict_incident_notification_initial.v1 /tmp/sf.json | head -5
rm -rf "$TMPV"
...
say "DONE — 1.9.0 built and verified."

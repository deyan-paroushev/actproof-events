...
Installation. The v1.0 module is distributed as a single-file Python module (`salt_erasure.py`) and requires Python 3.10 or later with the standard library only (no external dependencies for the core primitive). Drop the file into your project's Python path, or vendor it into your own package. A polished pip-installable distribution under the package name `openproof-salt-erasure` (importable as `openproof_salt_erasure`) is planned for v1.1; when that ships, the import statement in the example below changes from `from salt_erasure import ...` to `from openproof_salt_erasure import ...` with no other code changes required.
